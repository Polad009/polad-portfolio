  ...
  return (
    <div ref={sectionRef}>
      {t.projects.items.map((project) => (
        ...
        >
          <div className="max-w-[1400px] w-full mx-auto grid md:grid-cols-12 gap-8 md:gap-16 items-center">
            {/* Content */}
            <div className={project.image ? "md:col-span-7" : "md:col-span-12"}>
              {/* Number + Type */}
                  ...
                  <span
                    className={`text-[10px] tracking-[0.2em] uppercase ${
                      project.status === "Live" || project.status === "Canlı"
                        ? "text-green-400"
                        : project.status === "Development mərhələsində" || project.status === "In Development"
                        ? "text-amber-400"
                        : "text-white/40"
                    }`}
                  >
            ...
            {/* Image */}
            {project.image && (
              <div className="proj-anim md:col-span-5 w-full">
                <div className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-white/10 bg-white/[0.02] group">
                  <img
                    src={project.image}
                    alt={project.name}
